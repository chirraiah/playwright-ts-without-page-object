...
Given('Go to the letcode website', async function (this: ICustomWorld) {
  const { page } = this;
  await page?.goto(config.BASE_URL);
});

Given('click on SignUp button', async function (this: ICustomWorld) {
  const page = this.page;
  await page?.click('//app-root//a[@href="/signup"]');
});
